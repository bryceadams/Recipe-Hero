...
class RH_Admin_Settings {
	...
	public static function save() {
		...
		if ( empty( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'recipe-hero-settings' ) ) {
			die( __( 'Action failed. Please refresh the page and retry.', 'recipe-hero' ) );
		}

		// Trigger actions
		do_action( 'recipe_hero_settings_save_' . $current_tab );
		do_action( 'recipe_hero_update_options_' . $current_tab );
		do_action( 'recipe_hero_update_options' );

		// Clear any unwanted data
		delete_transient( 'recipe_hero_cache_excluded_uris' );

		self::add_message( __( 'Your settings have been saved.', 'recipe-hero' ) );

		// Flush rules
		flush_rewrite_rules();

		do_action( 'recipe_hero_settings_saved' );
	}
	 ...
	 * Settings page.
	 *
	 * Handles the display of the main recipe hero settings page in admin.
	 *
	 * @return void
	 */
	public static function output() {
		...
		do_action( 'recipe_hero_settings_start' );
		...
		if ( ! empty( $_GET['rh_error'] ) ) {
			self::add_error( stripslashes( $_GET['rh_error'] ) );
		}

		if ( ! empty( $_GET['rh_message'] ) ) {
			self::add_message( stripslashes( $_GET['rh_message'] ) );
		}

		self::show_messages();

		// Get tabs for the settings page
		$tabs = apply_filters( 'recipe_hero_settings_tabs_array', array() );
		...
		foreach ( $options as $value ) {
			...
			switch ( $value['type'] ) {
				...
				case 'title':
					...
					if ( ! empty( $value['id'] ) ) {
						do_action( 'recipe_hero_settings_' . sanitize_title( $value['id'] ) );
					}
					break;
				...
				case 'sectionend':
					if ( ! empty( $value['id'] ) ) {
						do_action( 'recipe_hero_settings_' . sanitize_title( $value['id'] ) . '_end' );
					}
					echo '</table>';
					if ( ! empty( $value['id'] ) ) {
						do_action( 'recipe_hero_settings_' . sanitize_title( $value['id'] ) . '_after' );
					}
					break;
				...
				case 'image_width' :
					...
					$size   = rh_get_image_size( $image_size );
					...
					if ( has_filter( 'recipe_hero_get_image_size_' . $image_size ) ) {
						$disabled_attr = 'disabled="disabled"';
						$disabled_message = "<p><small>" . __( 'The settings of this image size have been disabled because its values are being overwritten by a filter.', 'recipe-hero' ) . "</small></p>";
					}
					...
					?><tr valign="top">
						...
						<td class="forminp image_width_settings">
							...
							<label><input name="<?php echo esc_attr( $value['id'] ); ?>[crop]" <?php echo $disabled_attr; ?> id="<?php echo esc_attr( $value['id'] ); ?>-crop" type="checkbox" value="1" <?php checked( 1, $crop ); ?> /> <?php _e( 'Hard Crop?', 'recioe-hero' ); ?></label>
				...
				case 'single_select_page' :
					...
					?><tr valign="top" class="single_select_page">
						...
						<td class="forminp">
							<?php echo str_replace(' id=', " data-placeholder='" . __( 'Select a page&hellip;', 'recipe-hero' ) .  "' style='" . $value['css'] . "' class='" . $value['class'] . "' id=", wp_dropdown_pages( $args ) ); ?> <?php echo $description; ?>
						</td>
					</tr><?php
					break;
				...
				default:
					do_action( 'recipe_hero_admin_field_' . $value['type'], $value );
					break;
			}
		}
	}

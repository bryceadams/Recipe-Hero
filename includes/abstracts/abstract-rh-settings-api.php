<?php
/**
 * Admin Settings API used by Extensions
 *
 * @class       RH_Settings_API
 * @version     1.0.5
 * @package     Recipe Hero/Abstracts
 * @category    Abstract Class
 * @author      Recipe Hero
 */
abstract class RH_Settings_API {

	/** @var string The plugin ID. Used for option names. */
	public $plugin_id = 'recipe_hero_';
	...
	public function admin_options() { ?>
		...
		<h3><?php echo ( ! empty( $this->method_title ) ) ? $this->method_title : __( 'Settings', 'recipe-hero' ) ; ?></h3>
	...
	public function get_form_fields() {
		return apply_filters( 'recipe_hero_settings_api_form_fields_' . $this->id, $this->form_fields );
	}
	...
	public function process_admin_options() {
		...
		if ( count( $this->errors ) > 0 ) {
			...
			update_option( $this->plugin_id . $this->id . '_settings', apply_filters( 'recipe_hero_settings_api_sanitized_fields_' . $this->id, $this->sanitized_fields ) );
			$this->init_settings();
			return true;
		}
	}
	...
	public function generate_title_html( $key, $data ) {
		...
		?>
			...
			<h4 class="rh-settings-sub-title <?php echo esc_attr( $data['class'] ); ?>"><?php echo wp_kses_post( $data['title'] ); ?></h4>
	...
	public function validate_password_field( $key ) {
		...
		if ( isset( $_POST[ $this->plugin_id . $this->id . '_' . $key ] ) ) {
			$text = rh_clean( stripslashes( $_POST[ $this->plugin_id . $this->id . '_' . $key ] ) );
		}
	...
	public function validate_multiselect_field( $key ) {
		...
		if ( isset( $_POST[ $this->plugin_id . $this->id . '_' . $key ] ) ) {
			$value = array_map( 'rh_clean', array_map( 'stripslashes', (array) $_POST[ $this->plugin_id . $this->id . '_' . $key ] ) );
		} else {
			$value = '';
		}
